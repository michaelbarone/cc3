...
describe('Icon Management API', () => {
  ...
  describe('POST /api/admin/icons', () => {
    ...
    it('returns 403 when not admin', async () => {
      vi.mocked(verifyToken).mockImplementationOnce(async () => ({ id: 'regular-user', isAdmin: false }))
  ...
  describe('DELETE /api/admin/icons', () => {
    it('returns 401 when not authenticated', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/admin/icons?iconPath=/icons/test.webp', {
        method: 'DELETE'
      })

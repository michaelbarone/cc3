...
// Mock NextResponse
vi.mock('next/server', () => {
  const actual = vi.importActual('next/server') as any
  return {
    ...actual,
    NextResponse: {
...
describe('Authentication API', () => {
  ...
  describe('POST /api/auth/login', () => {
    ...
    describe('Password-based login', () => {
      it('authenticates user with valid credentials', async () => {
        ...
        vi.mocked(generateToken).mockResolvedValueOnce('valid_token')
    ...
    describe('Passwordless login', () => {
      it('authenticates user without password if no password set', async () => {
        ...
        vi.mocked(prisma.user.findUnique).mockResolvedValueOnce(passwordlessUser)
        vi.mocked(generateToken).mockResolvedValueOnce('valid_token')
        vi.mocked(prisma.user.update).mockResolvedValueOnce({
          ...passwordlessUser,
          lastLoginAt: new Date()
        ...
        const request = new NextRequest('http://localhost/api/auth/login', {
          method: 'POST',
          body: JSON.stringify({
            username: passwordlessUser.username
          })
        })
  ...
  describe('POST /api/auth/logout', () => {
    it('clears auth token cookie', async () => {
      ...
      // Verify cookie deletion response headers
      const headers = response.headers
      expect(headers.get('Set-Cookie')).toContain('auth_token=;')
    })

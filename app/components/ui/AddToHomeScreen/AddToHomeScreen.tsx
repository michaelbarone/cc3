...
const AddToMobileBrave = dynamic(() => import("./AddToMobileBrave"), {
  loading: () => <ModuleLoading />,
});
...
import useUserAgent from "@/app/lib/hooks/useUserAgent";
  ...
  useEffect(() => {
    ...
    if (addToHomeScreenPromptCookie !== "dontShow") {
      ...
      if (isMobile && !isStandalone) {
        ...
        } else if (userAgent === "Brave") {
          setDisplayPrompt("brave");
      ...
      {
        {
          ...
          brave: <AddToMobileBrave closePrompt={closePrompt} doNotShowAgain={doNotShowAgain} />,
          firefoxIos: (
            <AddToMobileFirefoxIos closePrompt={closePrompt} doNotShowAgain={doNotShowAgain} />
          ),
          chromeIos: (
  ...
  return (
    <>
      {displayPrompt !== "" ? (
        <>
          <div
            className="fixed top-0 left-0 right-0 bottom-0 bg-black/70 z-[3000]"
            onClick={closePrompt}
          >
            <Prompt />
          </div>

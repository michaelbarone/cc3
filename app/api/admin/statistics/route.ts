...
export async function GET() {
  try {
    ...
    const recentUsers = await prisma.user
      .findMany({
        where: {
          lastActiveUrl: { not: null },
        },
    ...
    const avgUrlsPerGroup = urlsPerGroup.length > 0 ? totalUrls / urlsPerGroup.length : 0;
    ...
    const urlMobileStats = await prisma.$queryRaw<
      Array<{ withMobile: bigint; desktopOnly: bigint }>
    >`
      SELECT
    ...
    const mostAccessedUrls = await prisma.$queryRaw<
      Array<{ title: string; url: string; count: bigint }>
    >`
    ...
    const statistics = {
      system: {
        users: {
          ...
          withPassword: Number(
            passwordStats.find((stat: PasswordStat) => stat.passwordHash !== null)?._count || 0,
          ),
          withoutPassword: Number(
            passwordStats.find((stat: PasswordStat) => stat.passwordHash === null)?._count || 0,
          ),
          adminRatio: userAdminStats.reduce(
            (acc: { admin: number; regular: number }, stat: UserAdminStat) => ({
              ...acc,
      ...
      userPreferences: {
        themeDistribution: Object.fromEntries(
          userThemeStats.map((stat: UserThemeStat) => [
            stat.themeMode || "default",
            Number(stat._count),
          ]),
        ),
        menuPositionDistribution: Object.fromEntries(
          userMenuStats.map((stat: UserMenuStat) => [
      ...
      urlGroups: {
        mostAssigned: groupsWithUserCount.map((group: UrlGroupWithCounts) => ({
          name: group.name,

...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Error getting app logo" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Error uploading app logo" }, { status: 500 });
  }
}
...
export async function DELETE() {
  try {
    ...
    if (!appConfig?.appLogo) {
      return NextResponse.json({ error: "App does not have a logo" }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: "Error deleting app logo" }, { status: 500 });
  }
}

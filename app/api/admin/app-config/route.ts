...
export async function GET() {
  try {
    ...
    if (!appConfig) {
      appConfig = await prisma.appConfig.create({
        data: {
          ...
          appLogo: null,
          loginTheme: "dark", // Already dark by default
    ...
    return NextResponse.json({ error: "Error getting app configuration" }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Error updating app configuration" }, { status: 500 });
  }
}
